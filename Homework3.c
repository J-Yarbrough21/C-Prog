...
double cplex_roots(double a, double b, double c);
...
Your program will ask the users for the coefficients a, b, and c,
and will then calculate and print both roots (solutions) for x, even if they are complex numbers.

Your program should  print an error if  a = 0.

If the roots are a pair of real numbers that differ, both should be displayed.

If there is only one (repeated) root, it should be displayed and the user should be told that it is a repeated root. •
 If the solution is a pair of complex numbers, they should be displayed in the form x = P + Qi and x = P −Qi
 where P and Q real numbers, and the symbol “i” represents the square root of -1.
